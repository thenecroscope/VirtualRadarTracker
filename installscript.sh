...
sudo chmod -R 777 $myhome/repos/VirtualRadarTracker/logs
sudo chown -R nobody:nogroup $myhome/repos/VirtualRadarTracker/logs
sudo setfacl -d -m user:nobody:rw $myhome/repos/VirtualRadarTracker/logs
...
echo -e '#!/usr/bin/env pwsh' | sudo tee --append /opt/VirtualRadarTracker/ServiceStarters/vrtslackbot.ps1
echo -e $myhome'/repos/VirtualRadarTracker/sbot.ps1' | sudo tee --append /opt/VirtualRadarTracker/ServiceStarters/vrtslackbot.ps1
...
chmod +x /opt/VirtualRadarTracker/ServiceStarters/vrtslackbot.ps1
...
echo -e "[Service]\nExecStart=/opt/VirtualRadarTracker/ServiceStarters/vrtslackbot.ps1\n[Install]\nWantedBy=default.target" | sudo tee --append /etc/systemd/system/vrtslackbot.service

systemctl enable vrtMilitary.service
systemctl enable vrtInteresting.service
systemctl enable vrtslackbot.service
